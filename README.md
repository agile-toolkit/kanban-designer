...
- **Keyboard shortcuts help** (`ShortcutsModal.tsx`) — a centered modal, opened via the header's `?` toolbar button or the `?` key (ignored while an `<input>`/`<textarea>` is focused), listing every keyboard shortcut already implemented elsewhere (card focus navigation, edit/delete, undo/redo) plus a Mouse & touch section for drag-and-drop and double-click-to-edit.
